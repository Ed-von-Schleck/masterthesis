...
The latter represents processes like fluctuations or initial transient processes, both having a short lifetime comparing to the secular processes.  %lifetime -> lifespan?
...
The variables of the unpertubated system represent the basis vectors of this vector space, denoting in the case of the variable $\mt{A}(t)$ as A-axis.  % zweiter Halbsatz: Was soll das heißen? In what case?
...
\end{align}
%
Here the Hermitian Liouville operator ${\mt{L} = \hbar^{-1} \comm{\mt{H}}{\mt{\bullet}}}$ is introduced, which is defined by acting onto an arbitrary operator.  % zweiter Halbsatz holprig, aber ich kann ihn nicht verbessern, weil ich ihn nicht verstehe :)
...
In reference to the secular part, in a pertubated system, a projection operator is defined in the Liouville space.  % Fühlt sich noch holprig an. 'In reference'? Woraus besteht die Referenz?
Starting with a set of arbitrary operators $\{\mt{C}_{i}\}$.  % Dieser Satz kein Verb
The choice of the operators is different for each investigated problem and unimportant at the moment.  % Wie hängt das mit dem vorigen Satz zusammen?
...
In the case of an invariant Hamiltonian with respect to time reversal symmetry and that the operators $\mt{A}_{i}$ and $\mt{C}_{k}$ of the expectation value in $\Omega_{il}$ have same signature under time reversal symmetry $\Omega_{il}$ vanishes.  % Dieser Satz ist ohne Interpunktion nicht verständlich, vermutlich auch zu lang.
...
The expectation value is performed with respect to the operators like $\mt{Q}\toket{\dot{\mt{A}}_{i}}$ instead of $\toket{\mt{A}_{i}}$, while only the reduced Liouville operator QLQ is considered.  % Was heißt 'The value is performed'?
...
Remember the sum in \eqref{eq:projection operator} summerizes over all operators $\mt{C}_{i}$, where the choice of them is different to each propblem and has to be done with respect to the investigated quantity, which is in our case the electrical conductivity.  % 'Remember' ist das falsche Verb, aber ich weiß nicht, genau, was Du sagen willst (resemble?)
...
Nevertheless, one remark has to be made on computing the conductivity.
...
Nothing bars us seperating the vector operator $\toket{\mt{J}(0)}$ into two pieces, one parallel and one perpendicular part, which are labeled as $\oket{\mt{J}_{\mid\mid}}$ and $\oket{\mt{J}_{\bot}}$, respectively.  % 'Nothing bars us' klingt nach zu wenig Motivation dafür, etwas zu tun.
